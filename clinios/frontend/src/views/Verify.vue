  ...
  <v-layout class="login-layout"> <!-- :style="{backgroundImage: 'url(' + background + ')'}"> -->
        ...
        <v-row align="center" justify="center">
          <v-col cols="11" sm="9" md="7" lg="4">
            <v-card class="elevation-1 login-form">
              <v-card-title>Practitioner Reset Password</v-card-title>
              <v-card-text>
              ...
              <v-card-actions class="pr-3 pb-3">
                <v-btn text small color="grey" :to="{name: 'login'}" active-class="no-active">Back</v-btn>
    ...
    <v-dialog v-model="dialog" persistent max-width="400">
      <v-card>
        ...
        <v-card-text>An email was just email. Please check your email inbox.</v-card-text>
        <v-card-actions>
          <div class="flex-grow-1"></div>
    ...
    methods: {
        verifyHandle() {
            if(this.$refs.verifyForm.validate()) {
                this.$http.post('/users/verify', { email: this.email}).then(res => {
                    if (res.data.error) {
                        this.$toast.error({
                          title: "Warning.", 
